...
export default function ServiceCard({ service, compact = false }) {
  ...
  if (compact) {
    return (
      <div
        className={`card-hover group rounded-2xl border bg-white p-6 shadow-sm ${
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          >
            <Icon className={`icon-pop h-6 w-6 ${isComingSoon ? 'text-gray-500' : 'text-primary'}`} />
          </div>
          {isComingSoon && (
  ...
  return (
    <div
      className={`card-hover group rounded-2xl border bg-white p-8 shadow-sm ${
    ...
    >
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        >
          <Icon className={`icon-pop h-7 w-7 ${isComingSoon ? 'text-gray-500' : 'text-primary'}`} />
        </div>
        {isComingSoon && (
