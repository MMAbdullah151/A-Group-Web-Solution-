...
export default function HeroAdCarousel({ ads }) {
  ...
  const goTo = (index) => {
    setActive((index + ads.length) % ads.length)
    setProgressKey((key) => key + 1)
  }
  ...
  return (
    <div
      className="relative mx-auto w-full max-w-xl animate-fade-up-delay-3 lg:mx-0 lg:max-w-none"
    ...
    >
      ...
      {current.overlay && (
        <div
          ...
          className="hero-ad-float overlay-pop absolute -bottom-4 -left-4 z-30 rounded-xl bg-white p-4 shadow-xl"
        >
        ...
        <div className="mt-5 space-y-3">
          ...
          <div className="mx-auto h-1 max-w-xs overflow-hidden rounded-full bg-white/20">
            <div
              ...
              style={{
                animation: paused ? 'none' : `progress-fill ${INTERVAL_MS}ms linear forwards`,
              }}
            />
          </div>
        </div>
